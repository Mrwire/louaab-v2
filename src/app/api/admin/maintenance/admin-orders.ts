...
const mapOrderFromApi = (raw: RawOrder): Order => ({
  id: raw?.id || raw?.orderNumber || `order-${Math.random().toString(36).slice(2)}`,
...
export const updateAdminOrderStatus = async (orderId: string, status: Order['status']): Promise<Order> => {
  ...
  if (orderId.startsWith('order-')) {
    throw new Error("Commande locale sans ID backend : rafraîchissez d'abord les commandes depuis l'API");
  }
