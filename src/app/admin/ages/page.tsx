...
export default function AdminAgesPage() {
  ...
  const loadAgeRanges = async () => {
    try {
      ...
      showNotification('Erreur lors du chargement des tranches d\'âges', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddAgeRange = async (ageRangeData: Partial<AgeRange>) => {
    if (!ageRangeData.label) {
      showNotification('Veuillez remplir le label', 'error');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        showNotification('Tranche d\'âge ajoutée avec succès !');
      ...
      showNotification(error instanceof Error ? error.message : 'Erreur lors de l\'ajout de la tranche d\'âge', 'error');
    }
  };

  const handleUpdateAgeRange = async (ageRangeData: Partial<AgeRange>) => {
    ...
    try {
      ...
      if (response.ok) {
        showNotification('Tranche d\'âge mise à jour avec succès !');
  ...
  const handleDeleteAgeRange = async (id: string) => {
    if (!confirm('Êtes-vous sûr de vouloir supprimer cette tranche d\'âge ?')) {
      return;
    }

    try {
      const response = await fetch(`${API_BASE_URL}/age-ranges/${id}`, {
        method: 'DELETE',
      });

      if (response.ok) {
        showNotification('Tranche d\'âge supprimée avec succès !');
        loadAgeRanges();
      } else {
  ...
  const renderIcon = (ageRange: AgeRange) => {
    if (ageRange.iconType === 'emoji') {
      return <span className="text-4xl">{ageRange.icon}</span>;
    } else if (ageRange.iconType === 'upload' && ageRange.iconUrl) {
      return (
        <Image
          src={ageRange.iconUrl}
          alt={ageRange.label}
          width={48}
          height={48}
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  {renderIcon(ageRange)}
                  <div>
                    <h3 className="font-semibold text-gray-900">{ageRange.label}</h3>
                    <p className="text-xs text-gray-500">{ageRange.slug}</p>
                  </div>
                </div>
                <button
                  onClick={() => setEditingAgeRange(ageRange)}
              ...
              <div className="space-y-2 text-sm text-gray-600">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {ageRange.ageMin} mois
                    {ageRange.ageMax ? ` - ${ageRange.ageMax} mois` : '+'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Ordre:</span>
                  <span className="font-medium">{ageRange.displayOrder}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Statut:</span>
                  <span
                  ...
                  >
                    {ageRange.isActive ? 'Actif' : 'Inactif'}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-4 flex gap-2">
                <button
                  onClick={async () => {
                    const updated = { ...ageRange, isActive: !ageRange.isActive };
                    try {
                      const response = await fetch(`${API_BASE_URL}/age-ranges/${ageRange.id}`, {
                        method: 'PATCH',
                        headers: { 'Content-Type': 'application/json' },
                ...
                >
                  {ageRange.isActive ? <EyeOff className="h-3 w-3 mx-auto" /> : <Eye className="h-3 w-3 mx-auto" />}
                </button>
                <button
                  onClick={() => handleDeleteAgeRange(ageRange.id)}
...
function AgeRangeModal({
  ...
  onClose: () => void;
  onSave: (data: Partial<AgeRange>) => void;
  isEditing: boolean;
  ...
  useEffect(() => {
    setFormData(ageRange);
    setIconType(ageRange.iconType || 'emoji');
  }, [ageRange]);
  ...
  const handleSave = () => {
    if (!formData.label) {
      alert('Veuillez remplir le label');
      return;
    }
    ...
    const finalData: Partial<AgeRange> = {
      ...formData,
      iconType,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-teal-50 to-cyan-50 rounded-t-2xl">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-gray-900">
              {isEditing ? 'Modifier la tranche d\'âge' : 'Ajouter une tranche d\'âge'}
            </h2>
            <button
              onClick={onClose}
              ...
              <input
                ...
                onChange={(e) => {
                  const slug = e.target.value
                    ...
                    .replace(/-+/g, '-')
                    .replace(/^-|-$/g, '');
                  setFormData({ ...formData, label: e.target.value, slug });
                }}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
                placeholder="Ex: 0-12 mois"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, ageMax: e.target.value ? parseInt(e.target.value) : null })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500"
                  min="0"
                  ...
                  {predefinedEmojis.map((emoji) => (
                    <button
                      ...
                      className={`p-2 rounded-lg text-2xl hover:bg-white transition-colors ${
                        formData.icon === emoji ? 'bg-teal-100 ring-2 ring-teal-500' : ''
                      }`}
                    >
                      {emoji}
                    </button>
        ...
        <div className="p-6 border-t border-gray-200 bg-gray-50 rounded-b-2xl flex gap-3">
          <button
            onClick={handleSave}
            className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-teal-500 to-cyan-600 px-6 py-3 text-white font-semibold hover:from-teal-600 hover:to-cyan-700 transition-all shadow-lg"
          >
