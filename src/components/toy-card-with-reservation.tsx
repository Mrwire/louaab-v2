...
export default function ToyCardWithReservation({ toy, priority = false }: ToyCardWithReservationProps) {
  ...
  return (
    <>
      <div onClick={handleNavigate} role="button" tabIndex={0} className="group cursor-pointer overflow-hidden rounded-2xl bg-white shadow-sm transition hover:shadow-xl">
        {/* Image */}
        <div className="relative aspect-square overflow-hidden">
          ...
          <div className="absolute inset-x-0 bottom-4 flex justify-center gap-2 opacity-80 transition duration-300 hover:opacity-100">
            ...
            >
              <Heart size={18} className={isFavorite(String(toy.id)) ? 'fill-white' : ''} />
            </button>
            <button onClick={(e) => { e.stopPropagation(); router.push(`/jouets/${toy.slug}`); }} className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-charcoal shadow-lg transition hover:bg-[#1897aa] hover:text-white">
              <Eye size={18} />
            </button>
        ...
        <div className="p-6">
          ...
          <div className="mt-4 flex gap-2">
            <button onClick={(e) => { e.stopPropagation(); router.push(`/jouets/${toy.slug}`); }} className="flex-1 rounded-xl border-2 border-gray-200 px-4 py-2 text-center text-sm font-semibold text-charcoal transition hover:bg-gray-50">
              Voir détails
            </button>
