    ...
    async updateStatus(id, status) {
        ...
        if (status === order_entity_1.OrderStatus.CONFIRMED) {
            ...
            for (const item of order.items) {
                const toy = await this.toyRepository.findOne({ where: { id: item.toy.id } });
                if (toy) {
                    const physicalStock = Number((_c = toy.stockQuantity) !== null && _c !== void 0 ? _c : 0);
                    const available = Number((_d = toy.availableQuantity) !== null && _d !== void 0 ? _d : 0);
                    const baseAvailable = available > 0 ? available : physicalStock;
                    ...
                    toy.timesRented = Math.max(0, ((_e = toy.timesRented) !== null && _e !== void 0 ? _e : 0) + item.quantity);
    ...
    async reset(id) {
        ...
        for (const item of order.items) {
            ...
            if ([order_entity_1.OrderStatus.CONFIRMED, order_entity_1.OrderStatus.DELIVERED].includes(previousStatus)) {
                ...
                const currentAvailable = Number((_b = toy.availableQuantity) !== null && _b !== void 0 ? _b : 0);
